                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: '12px', overflow: 'auto', flex: 1, alignContent: 'start' }}>
                    {filteredItems.map(item => (
                        <div key={item.id} className="card" onClick={() => addToCart(item, tab === 'services' ? 'service' : 'product')}
                            style={{ cursor: 'pointer', transition: 'all 150ms', minHeight: '240px', display: 'flex', flexDirection: 'column' }}
                            ...
                            <div className="card__body" style={{ padding: '16px', flex: 1, display: 'flex', flexDirection: 'column' }}>
                                <div style={{ width: '100%', height: '160px', borderRadius: '16px', overflow: 'hidden', marginBottom: '12px', background: 'rgba(243,244,246,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                    {item.image_url ? (
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '16px', gap: '12px' }}>
                                    <div style={{ display: 'flex', flexDirection: 'column' }}>
                                        <span style={{ fontSize: '12px', color: 'var(--text-muted)', marginBottom: '4px' }}>Precio</span>
                                        <span style={{ fontWeight: 700, color: 'var(--primary-600)' }}>{fmt(item.price)}</span>
                                    </div>
                                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}>
                                        <span style={{ fontSize: '12px', color: 'var(--text-muted)', marginBottom: '4px' }}>Existencias</span>
